...
var DemoList = class extends import_coc.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "demo_list";
    this.description = "CocList for coc-vale";
  ...
  async loadItems(context) {
    return [
      {
        label: "coc-vale list item 1",
        data: { name: "list item 1" }
      },
      {
        label: "coc-vale list item 2",
...
async function activate(context) {
  import_coc2.window.showMessage(`coc-vale works!`);
  context.subscriptions.push(
    import_coc2.commands.registerCommand("coc-vale.Command", async () => {
      import_coc2.window.showMessage(`coc-vale Commands works!`);
    }),
    import_coc2.listManager.registerList(new DemoList(import_coc2.workspace.nvim)),
    import_coc2.sources.createSource({
      name: "coc-vale completion source",
      doComplete: async () => {
